...
// Block-diagonal mat-mul i.e C = F_blk * B  where F is 69 × (4×4) blocks
...
static void predict_P_blockdiag(double *F1, double *Q1,
    ...
    /* Add Q (block-diagonal Q1 on each 4×4 block) */
    for (int b=0;b<69;b++) {
        int base = b*4;
        for (int r=0;r<4;r++)
            for (int c=0;c<4;c++)
...
/* H*x: exploit block-sparse H (picks rows 0,4,8 of each 12-row joint) */
static void H_times_x(double *x, double *Hx) {
    for (int j=0;j<N_JOINTS;j++) {
        Hx[j*3+0] = x[j*N_JOINT+0];   // px 
        Hx[j*3+1] = x[j*N_JOINT+4];   // py 
        Hx[j*3+2] = x[j*N_JOINT+8];   // pz 
    }
}
...
// H*PHt: S = H*PHt + R  (69x276 x 276x69 = 69x69)
...
// H*P: exploit H sparsity
static void H_times_P(double *P, double *HP) {
    static const int offsets[3] = {0, 4, 8};
    for (int j=0;j<N_JOINTS;j++) {
        for (int k=0;k<3;k++) {
            int src_row = j*N_JOINT + offsets[k];
            int dst_row = j*3 + k;
            memcpy(&HP[dst_row*N_STATE], &P[src_row*N_STATE],
...
static void lkf_predict_fast(double *x, double *P,
                               ...
                               double *F1T) {
    /* x_pred = F*x: block-diagonal multiply, each 4x1 block */
    for (int b=0;b<69;b++) {
        int row=b*4;
...
static void lkf_update_fast(double *x_pred, double *P_pred,
    ...
    /* Solve S * K^T = PHt^T  -> K^T via Cholesky (69x69 solve, fast) */
    asm_mat_transpose(PHt, Kt, N_STATE, N_MEAS);  /* Kt = PHt^T (69×276) */
    mat_copy(Ychol, S, (long)N_MEAS*N_MEAS);
    asm_cholesky_solve(Ychol, Kt, Kt, tmp1, N_MEAS, N_STATE);
    asm_mat_transpose(Kt, K, N_MEAS, N_STATE);    /* K (276×69) */

    /* y = z - H*x_pred  (exploit H sparsity) */
    H_times_x(x_pred, Hxp);
    asm_mat_sub(z, Hxp, Hxp, N_MEAS, 1);

    /* x_upd = x_pred + K*y  (276x69 x 69x1 = 276x1, general) */
    asm_mat_mul(K, Hxp, x_upd, N_STATE, N_MEAS, 1);
    asm_mat_add(x_pred, x_upd, x_upd, N_STATE, 1);

    // P_upd = P_pred - K*(H*P_pred)
    /* Step 1: HP = H * P_pred*/
    H_times_P(P_pred, tmp2);                

    /* Step 2: KHP = K * HP  (276x69 * 69x276 = 276x276) */
    asm_mat_mul(K, tmp2, tmp1, N_STATE, N_MEAS, N_STATE);

    /* Step 3: P_upd = P_pred - K*H*P_pred */
    asm_mat_sub(P_pred, tmp1, P_upd, N_STATE, N_STATE);
