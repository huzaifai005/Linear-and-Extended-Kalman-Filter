...
static void cartesian_to_spherical(double *z_cart, double *z_sph)
{
    for (int j = 0; j < N_JOINTS; j++)
    {
        ...
        double rho = asm_newton_sqrt(px * px + py * py);
        double r = asm_newton_sqrt(px * px + py * py + pz * pz);
        double eps = 1e-9;
        z_sph[j * 3 + 0] = r;
        z_sph[j * 3 + 1] = (r > eps) ? asm_cordic_atan2(py, px) : 0.0;
        z_sph[j * 3 + 2] = (r > eps) ? asm_cordic_atan2(pz, rho) : 0.0;
    }
}
                       ...
                       double *Ychol, double *K)
{
    ...
    // IKH = I - K*Hk  (276×69 × 69×276 = 276×276)
    asm_mat_mul(K, Hk, IKH, N_STATE, N_MEAS, N_STATE);
    mat_negate(IKH, (long)N_STATE * N_STATE);
    add_identity(IKH, N_STATE);

    // P_upd = IKH*P_pred*IKH^T + K*R*K^T  (Joseph form)
    ...
    // Step A: tmp1 = K * R  (276×69 × 69×69 = 276×69)
    asm_mat_mul(K, R, tmp1, N_STATE, N_MEAS, N_MEAS);

    // Step B: tmp2 = K^T  (69×276, write into start of tmp2 276×276 buffer)
    asm_mat_transpose(K, tmp2, N_STATE, N_MEAS);

    // Step C: tmp1 = (K*R) * K^T  (276×69 × 69×276 = 276×276)
    asm_mat_mul(tmp1, tmp2, IKH, N_STATE, N_MEAS, N_STATE);

    // P_upd += K*R*K^T
    asm_mat_add(P_upd, IKH, P_upd, N_STATE, N_STATE);
...
int main(int argc, char *argv[])
{
    ...
    for (int k = 1; k < nframes; k++)
    {
        ...
        // Jacobian
        memset(Hk, 0, (long)N_MEAS * N_STATE * sizeof(double));
